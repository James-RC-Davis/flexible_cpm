...
covars_path = strcat(tilda_data_dir,folder_name,'\covars_scaled.csv');
...
macronet_masks = generate_macronet_masks();
...
model_info = struct('predictor_vars_file', predictors_path, 'target_covars_path',...
    ...
    {adjust_stage}', 'k', k, 'iterations', iterations, 'fs_thresh_type',...
...
clearvars -except all_behav all_mats all_covars k thresh thresh_type freq_thresh...
    adjust_stage iterations model_info perm_iterations run_permutation cat_covars macronet_masks
...
if strcmp(adjust_stage,'relate')
    ...
    [~, ~, int_pos_ntwrk, int_neg_ntwrk,...
        ...
        ~] = macronets_prep_parameters_arrays_CPM(all_mats,...
...
[~, ~, int_pos_ntwrk, int_neg_ntwrk,...
    ...
    slope_combined_covars] = macronets_prep_parameters_arrays_CPM(all_mats,...
    all_covars, k, iterations, adjust_stage);
end  
...
%         macronets_run_flexible_CPM(all_behav, all_mats, all_covars, k, ...
...
%     
    ...
    if k==length(all_behav)
        [behav_pred_pos_all(:,i), behav_pred_neg_all(:,i), ...
            ...
            parameters_combined, pos_mask, neg_mask, no_node, no_covars] = ...
            macronets_run_flexible_CPM(all_behav, all_mats, all_covars, k, ...
            thresh_type, thresh, adjust_stage, cat_covars, macronet_masks);
    else
        [behav_pred_pos_all(:,i), behav_pred_neg_all(:,i), ...
            behav_pred_combined_all(:, i), parameters_pos, parameters_neg,...
            parameters_combined, pos_mask, neg_mask, no_node, no_covars] = ...
...
if strcmp(run_permutation, 'yes')
    ...
    if iterations > 1
        ...
        macronets_CPM_permutation_test(all_behav, all_mats, all_covars, k, ...
        thresh_type,thresh, adjust_stage, mean_results.R_pos, ...
        mean_results.R_neg, mean_results.R_combined, perm_iterations, corr_type, cat_covars, macronet_masks);
    else 
        [perm_p_pos, perm_p_neg, perm_p_combined] = ...
        macronets_CPM_permutation_test(all_behav, all_mats, all_covars, k, ...
        thresh_type, thresh, adjust_stage, R_pos, R_neg, ...
        R_combined, perm_iterations, cat_covars, corr_type, macronet_masks);
    end
