...
macronet_within_masks = {zeros(268, 268); zeros(268, 268); zeros(268, 268); zeros(268, 268)};
...
for m =1:length(macro_network_nodes)
    for i=1:length(macro_network_nodes{m})
        for j=i+1:length(macro_network_nodes{m})
            macronet_within_masks{m}(i,j)=1;
        end
    end
    plot(macronet_within_masks{m})
end
